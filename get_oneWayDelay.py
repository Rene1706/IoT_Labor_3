...
def calculate_packet_counts_udp(client_file, server_file, output_file):
    client_counts = [(0,0)]
    server_counts = [(0,0)]
    # Read client.txt file and count client packets
    with open(client_file, 'r') as file:
        for line in file:
            c_timestamp, _, _ = line.split()
            client_counts.append((c_timestamp, client_counts[-1][1] + 1 ))

    # Read server.txt file and count server packets
    with open(server_file, 'r') as file:
        for line in file:
            s_timestamp, _, _ = line.split()
            server_counts.append((s_timestamp, server_counts[-1][1] + 1))
    del server_counts[0]
    del client_counts[0]
    print(len(server_counts))
    print(len(client_counts))
...
def calculate_one_way_delay_udp(client_file, server_file, output_file):
    # Read the client.txt file
    client_data = []
    server_data = []
    ...
    # Calculate one-way delay for each sequence number
    try:
        for client, server in zip(client_data, server_data):
            one_way_delays.append(client[0]-server[0])
    except:
        print(client, server)
    #print(one_way_delays)
    print("MEAN:", np.mean(one_way_delays[:4000]))
...
calculate_packet_counts_udp(client_file, server_file, output_file)
...
calculate_one_way_delay_udp(client_file, server_file, output_file)
